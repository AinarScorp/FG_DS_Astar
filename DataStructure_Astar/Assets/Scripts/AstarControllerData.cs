...
[Serializable]
public class AstarControllerData
{
    ...
    public AstarControllerData() { }
    public AstarControllerData(int width, int height, Vector2 cellSize)
    {
        this.width = width;
        this.height = height;
    ...
    public AstarControllerData Load()
    {
        string jsonGridData = File.ReadAllText(Application.dataPath + FILE_NAME);
        return JsonUtility.FromJson<AstarControllerData>(jsonGridData);
    }
